...
-export([createMonitor/0, addStation/3, addValue/5, removeValue/4, getOneValue/4, getStationMean/3, getDailyMean/3, getHourlyMean/4, getMeasureMaxValue/3,getLastMeasurment/3]).
...
getLastMeasurment(Identifier, Type, Table) ->
  Last = fun(Beg, Date, Type, Value, Station, Tail) -> lastMeasurment(Beg, Date, Type, Value, Station, Tail) end,
  getStation(Identifier, Last, 0, Type, 0, Table, []).

lastMeasurment(_,_,_,_,{},_) -> null;
lastMeasurment(_,_,Type,_,Station,_) ->
  {_,_,Table} = Station,
